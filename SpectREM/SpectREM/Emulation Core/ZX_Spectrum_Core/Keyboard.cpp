...
ZXSpectrum::KEYBOARD_ENTRY ZXSpectrum::keyboard_lookup[] =
{
...
{
    for (uint32_t i = 0; i < static_cast<int>(sizeof(keyboard_lookup) / sizeof(keyboard_lookup[0])); i++)
    {
        if (keyboard_lookup[i].key == key)
        {
            KEYBOARD_ENTRY& entry = keyboard_lookup[i];
            if (entry.mapEntry1 != -1 && entry.mapBit1 != -1)
            {
                keyboard_map[entry.mapEntry1] &= ~(1 << entry.mapBit1);
            }
            if (entry.mapEntry2 != -1 && entry.mapBit2 != -1)
            {
                keyboard_map[entry.mapEntry2] &= ~(1 << entry.mapBit2);
            }
            if (key == ZXSpectrumKey::Key_CapsLock)
            {
                keyboard_capslock_pressed = true;
            }
            break;
        }
    }
...
void ZXSpectrum::keyboardCheckCapsLockStatus()
{
    ...
    if (keyboard_capslock_pressed && keyboard_capslock_frames > 2)
    {
        keyboard_map[0] |= 0x01;
        keyboard_map[3] |= 0x02;
        keyboard_capslock_pressed = false;
        keyboard_capslock_frames = 0;
    }
    else if (keyboard_capslock_pressed)
    {
        keyboard_capslock_frames ++;
    }
}
